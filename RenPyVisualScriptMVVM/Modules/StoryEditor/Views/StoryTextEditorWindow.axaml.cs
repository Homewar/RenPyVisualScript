...
public partial class StoryTextEditorWindow : Window
{
    ...
    public StoryTextEditorWindow()
    {
        ...
        Opened += async (_, _) =>
        {
            if (DataContext is StoryTextEditorWindowViewModel vm)
    ...
    private void OnStoryTextEditorTextChanged(object? sender, System.EventArgs e)
    {
    ...
    {
        text ??= string.Empty;
        if (string.Equals(editor.Text, text, System.StringComparison.Ordinal))
            return;
...
public sealed class StoryFontTagColorizer : DocumentColorizingTransformer
{
    ...
    private bool TryResolveFontFile(string fontName, out string fontFilePath)
    {
        ...
        foreach (var candidate in candidates)
        {
            ...
            {
                // Ignore invalid font paths from story markup.
            }
        }
